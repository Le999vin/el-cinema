...
export const MovieCard = ({ movie }: { movie: MovieSummary }) => (
  <Card className="overflow-hidden p-0">
    <div className="relative h-72 w-full bg-[color:var(--panel-soft)]">
      {movie.posterUrl ? (
        <Image src={movie.posterUrl} alt={movie.title} fill className="object-cover" sizes="(max-width: 1024px) 50vw, 25vw" />
      ) : (
      ...
      <div className="absolute bottom-3 left-3 right-3 flex items-end justify-between">
        <div>
          <h3 className="font-[family-name:var(--font-display)] text-2xl leading-tight text-white">{movie.title}</h3>
          <p className="text-xs text-zinc-300">{movie.releaseDate ?? "TBA"}</p>
        </div>
        <Link href={`/movies/${movie.id}`} className="text-xs text-[color:var(--accent-soft)]">
          Open
        </Link>
      </div>
    </div>
